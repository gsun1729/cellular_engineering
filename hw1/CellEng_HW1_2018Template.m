...
[WT_beta, WT_res] = nlinfit(L0, WT, @, beta0),    % WT_beta = [proportionality constant, Kd] , ** This needs to be filled in to run **

 %Plots experimental wild-type data
 figure;
  semilogx( );  % what you are plotting
  xlabel(' ', 'FontSize', 16);
  ylabel(' ', 'FontSize', 16);
  title(' ', 'FontSize', 18);
  axis([ ]);
...
%Plots fit of wild-type data
 hold on;
  p =  ; % this is your array of ligand points to use for fitting
  WT_fit =  % this will be a function of WT_beta(1) WT_beta(2), and p
  semilogx( ); % what you are plotting
  legend( );
 hold off;
...
function signal = binding(beta, L)
      Var1   = beta(1);
      Var2   = beta(2);
      signal  =  ; % Will be a function of L and what you name as Variables (Var1 and Var2), equation from class
return;