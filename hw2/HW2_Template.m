...
%pts         =                           % # of s.s. evaluations
%stim        = zeros(pts,3);             % A different stimulus range will be used for each enzyme
%stim(:,1)   = logspace( ,  , pts)';     % MAPKhas a narrow range due to ultrasensitivity
%stim(:,2)   = logspace( , , pts)';      % MAPKK has a medium range
%stim(:,3)   = logspace( , , pts)';      % MAPKKK has a broad stimulus range
%v           = zeros(pts,3);             % values of key parameters at s.s.
